...
import { Member } from 'apps/sneakers-api/src/libs/dto/member/member';
import { Product } from 'apps/sneakers-api/src/libs/dto/product/product';
import { MemberStatus, MemberType } from 'apps/sneakers-api/src/libs/enums/member.enum';
import { ProductStatus } from 'apps/sneakers-api/src/libs/enums/product.enum';
...
export class BatchService {
	...
	public async batchTopProducts(): Promise<void> {
		...
		const promisedList = products.map(async (ele: Product) => {
			...
			const rank = productLikes + productViews;
